...
-- 2. Importando as primeiras 25 linhas da tabela
select * 
from funcionarios 
limit 25;
...
update funcionarios set	cargo = replace(cargo,'funcionarios Accountant IV','Employee Accountant IV')
where cargo = 'funcionarios Accountant IV';

update funcionarios set	cargo = replace(cargo,'funcionarios Accountant I','Employee Accountant I')
where cargo = 'funcionarios Accountant I';

update funcionarios set	cargo = replace(cargo,'funcionarios Accountant II','Employee Accountant II')
where cargo = 'funcionarios Accountant II';

update funcionarios set	cargo = replace(cargo,'funcionarios Accountant III','Employee Accountant III')
where cargo = 'funcionarios Accountant III';
...
-- 14. Quantos funcionários foram admitidos por ano?
select 
...
-- 21. Quantos funcionários por região (Brasil)?
select
		localizacao,
		pais,
...
-- 22. Quantos funcionários por província (Canadá)?
select
		localizacao,
		pais,
		count(id) as qtde_funcionarios
...
-- 23. Qual região/província tem o maior salário médio?
select
		localizacao,
		pais,
...
-- 24. Distribua os funcionários por país e sexo
select	
		pais,
		sexo,
		count(id) as qtde_funcionarios
...
-- 25. Quantos cargos únicos existem?
select
		count(distinct cargo)
from funcionarios;


-- 26. Quais os 10 cargos mais comuns?
select	
		cargo,
		count(*) as qtde_funcionarios
from funcionarios
...
-- 27. Qual cargo tem o maior salário médio?
select
		cargo,
		round(avg(salario),2) as media_salario
from funcionarios
...
-- 28. Identifique cargos que aparecem em apenas um departamento
select	
		cargo
from funcionarios
group by cargo
...
-- 29. Mostre todos os funcionários do Brasil.
select
		nome,
		cargo,
		pais
...
-- 30. Liste os funcionários do departamento “Computadores” com salário acima de 70.000.
select 
		nome,
		departamento,
		salario
...
-- 31. Exiba funcionários contratados após 2013.
select
		nome,
		cargo,
		data_admissao
...
-- 32. Liste os 10 primeiros funcionários ordenados pelo salário do maior para o menor.
select
		nome,
		salario
from funcionarios
...
-- 33. Mostre o nome, departamento e salário dos 5 funcionários mais bem pagos.
select
		nome,
		departamento,
		salario
...
-- 34. Calcule o salário médio de todos os funcionários.
select
		nome,
		round(avg(salario),2)
from funcionarios
